...
@Component({
  selector: 'app-confirmar-reseteo-username',
  templateUrl: './confirmar-reseteo-username.component.html',
  styleUrls: ['./confirmar-reseteo-username.component.css']
})
export class ConfirmarReseteoUsernameComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    if (this.email && this.phoneNumber) {
      ...
      const message = 'Su usuario ha sido restablecido con exito.';
  ...
  */
  ...
  enviarConfirmacion(email: string, subject: string, message: string): void {
    ...
    // Primer envío de la confirmación por correo electrónico y SMS
    this.http.post(`${this.apiUrl}/send-confirmation`, payload).subscribe(
      confirmResponse => {
